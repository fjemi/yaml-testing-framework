...
class Data:
  body: Body | None = None
  test_files: List[get_file_paths.File_Paths] | None = None
...
async def get_test_data_from_yml(path: get_file_paths.File_Paths,) -> dict:
  test_data = {}

  if not os.path.exists(str(path.yml_path)):
    return test_data
...
async def get_module_resources_and_tests(
  path: get_file_paths.File_Paths,) -> Module:
  module = Module()

  module.function_names = await get_function_definitions.main(
    module_path=str(path.py_path), output_attributes="name")
  ...
  module.module = await get_module.main(path=path.py_path)
...
async def add_test_resources_to_module(
  module: ModuleType,
  path: get_file_paths.File_Paths,
) -> ModuleType:
  ...
  for resource_path in path.test_resources:
    ...
    resource_tree = resource_path.replace(path.directory, "")
    ...
    resource_module = await get_module.main(path=resource_path)
...
async def setup_tests(data: Data) -> Data:
  ...
  for path in data.test_files:
    ...
    for function in module.functions:
      ...
      # for test in function.tests:
      #   test = await execute_test(test=test)
      #   pass
...
async def main(
  project_path: str | None = None,
  exclude_match: List[str] | None = None,
  yml_suffix: str | None = None,
  yml_prefix: str | None = None,
  test_resources_directory: str | None = None,
  text: str | None = None,
  ...
  data.test_files = await get_file_paths.main(
    ...
    exclude_match=data.body.exclude_match,
    yml_suffix=data.body.yml_suffix,
    yml_prefix=data.body.yml_prefix,
    test_resources_directory=data.body.test_resources_directory,
  )
